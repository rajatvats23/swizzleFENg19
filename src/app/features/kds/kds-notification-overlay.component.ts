...
@Component({
  ...
  template: `
    <div class="notification-container" *ngIf="notifications.length > 0">
      <div class="notification-card" @slideIn *ngFor="let order of notifications">
        <div class="notification-header">
          <div class="notification-title">
            <mat-icon class="material-symbols-outlined" color="warn">notifications_active</mat-icon>
        ...
        <div class="notification-content">
          <div class="item-summary">
            <span *ngFor="let item of order.items; let last = last">
              {{item.quantity}}× {{item.product.name}}{{!last ? ', ' : ''}}
            </span>
          </div>
        </div>
        
        <div class="notification-actions">
          <button mat-icon-button matTooltip="Print Ticket" (click)="printOrder(order)">
            <mat-icon class="material-symbols-outlined">print</mat-icon>
          </button>
          <button mat-icon-button matTooltip="View Details" (click)="viewOrderDetails(order)">
            <mat-icon class="material-symbols-outlined">visibility</mat-icon>
          </button>
          <button mat-icon-button matTooltip="Dismiss" (click)="dismissNotification(order._id)">
    ...
    <div class="sound-toggle" [matBadge]="notifications.length" 
         [matBadgeHidden]="notifications.length === 0"
         matBadgeColor="warn" matBadgeOverlap="true">
      <button mat-mini-fab color="basic" (click)="toggleSound()">
        <mat-icon class="material-symbols-outlined">{{soundEnabled ? 'volume_up' : 'volume_off'}}</mat-icon>
      </button>
    </div>
  `,
  styles: [`
